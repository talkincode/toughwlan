...
class Config():
    ...
    def __init__(self, conf_file=None, **kwargs):
        ...
        self.ac = ConfigDict(**{k: v for k, v in self.config.items("ac") if k not in self.defaults})
        self.portal = ConfigDict(**{k: v for k, v in self.config.items("portal") if k not in self.defaults})
        self.radius = ConfigDict(**{k: v for k, v in self.config.items("radius") if k not in self.defaults})
    ...
    def update(self):
        ...
        for k, v in self.ac.iteritems():
            if k not in self.defaults:
                self.config.set("ac", k, v)

        for k, v in self.portal.iteritems():
            if k not in self.defaults:
                self.config.set("portal", k, v)

        for k, v in self.radius.iteritems():
            if k not in self.defaults:
                self.config.set("radius", k, v)
