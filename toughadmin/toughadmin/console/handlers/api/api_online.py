...
class OnlineQueryHandler(ApiHandler):
    ...
    def post(self):
        ...
        session = self.db.query(models.TraOnline).filter_by(session_id=session_id)
...
class OnlineExistsHandler(ApiHandler):
    ...
    def post(self):
        ...
        _exists = self.db.query(models.TraOnline.id).filter_by(session_id=session_id).count() > 0
