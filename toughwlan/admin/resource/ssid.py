...
from toughwlan.admin.resource import ssid_form
...
class SsidHandler(BaseHandler):
    ...
    def get(self):
        self.render("ssid_list.html",
                      page_data=self.get_page_data(self.db.query(models.TrwSsid)))
...
class AddHandler(BaseHandler):
    @cyclone.web.authenticated
    def get(self):
        domains = [(m.domain_code, m.domain_desc) for m in self.db.query(models.TrwDomain)]
    ...
    def post(self):
        ...
        if self.db.query(models.TrwSsid.id).filter_by(ssid=form.d.ssid).count() > 0:
