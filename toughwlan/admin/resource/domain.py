...
class DomainDetailHandler(BaseHandler):
    ...
    def get(self):
        ...
        attrs = self.db.query(models.TrwDomainAttr).filter_by(domain_code=domain.domain_code)
...
class DomainAttrAddHandler(BaseHandler):
    ...
    def get(self):
        ...
        if self.db.query(models.TrwDomain).filter_by(domain_code=domain_code).count() == 0:
    ...
    def post(self):
        ...
        self.db.commit()
        domain_id = self.db.query(models.TrwDomain.id).filter_by(domain_code=form.d.domain_code).scalar()
...
class DomainAttrDeleteHandler(BaseHandler):
    ...
    def get(self):
        ...
        self.db.commit()
        domain_id = self.db.query(models.TrwDomain.id).filter_by(domain_code=domain_code).scalar()
        self.redirect("/domain/detail?domain_id=%s" % domain_id)
