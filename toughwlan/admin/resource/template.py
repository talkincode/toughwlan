...
class TemplateDetailHandler(BaseHandler):
    ...
    def get(self):
        tpl_id = self.get_argument("tpl_id")
        tpl = self.db.query(models.TrwTemplate).get(tpl_id)
        attrs = self.db.query(models.TrwTemplateAttr).filter_by(tpl_name=tpl.tpl_name)
...
class TemplateAttrAddHandler(BaseHandler):
    ...
    def get(self):
        ...
        if self.db.query(models.TrwTemplate).filter_by(tpl_name=tpl_name).count() == 0:
    ...
    def post(self):
        ...
        tpl_attr = models.TrwTemplateAttr()
        ...
        self.db.commit()
        tpl_id = self.db.query(models.TrwTemplate.id).filter_by(tpl_name=form.d.tpl_name).scalar()
...
class TemplateAttrUpdateHandler(BaseHandler):
    ...
    def get(self):
        ...
        attr = self.db.query(models.TrwTemplateAttr).get(attr_id)
    ...
    def post(self, *args, **kwargs):
        ...
        tpl_attr = self.db.query(models.TrwTemplateAttr).get(form.d.id)
        tpl_attr.attr_name = form.d.attr_name
        tpl_attr.attr_value = form.d.attr_value
        tpl_attr.attr_desc = form.d.attr_desc
...
class TemplateAttrDeleteHandler(BaseHandler):
    ...
    def get(self):
        ...
        self.db.query(models.TrwTemplateAttr).filter_by(id=attr_id).delete()

        self.db.commit()
        tpl_id = self.db.query(models.TrwTemplate.id).filter_by(tpl_name=tpl_name).scalar()
        self.redirect("/template/detail?tpl_id=%s" % tpl_id)
