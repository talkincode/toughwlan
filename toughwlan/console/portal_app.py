...
class Application(cyclone.web.Application):
    ...
    def init_route(self):
        handler_path = os.path.join(os.path.abspath(os.path.dirname(__file__)), "portal")
        load_handlers(handler_path=handler_path, pkg_prefix="toughwlan.console.portal")

def run(config, log=log):
    app = Application(config, log)
